...
// Purpose: 
//-----------------------------------------------------------------------------
void ClientModeTFBase::Disable( void )
{
}
...
//-----------------------------------------------------------------------------
int	ClientModeTFBase::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	if ( engine->Con_IsVisible() == false )
	{
		// Let hint system snag first escape key release
		if ( down && ( keynum == K_ENTER ) && HintSystemEscapeKey() )
		{
			return 0;
		}
