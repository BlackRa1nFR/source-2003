...
void CVehicleMotorcycleControlPanel::OnTick()
{
	...
	{
		Q_snprintf( buf, 256, "Driven by %s", pCycle->GetDriverPlayer()->GetPlayerName() );
	...
	if ( pCycle->GetOwner() == C_BaseTFPlayer::GetLocalPlayer() )
	{
		if (nPassengerCount == nMaxPassengerCount)
		{
			...
			C_BaseTFPlayer *pPlayer = static_cast<C_BaseTFPlayer*>(pCycle->GetPassenger( VEHICLE_DRIVER ));
