...
DECLARE_MESSAGE( GetTechnologyTreeDoc(), Technology )

// Create object singleton on stack
CTechnologyTreeDoc& GetTechnologyTreeDoc()
{
	static CTechnologyTreeDoc g_TechnologyTreeDoc;
	return g_TechnologyTreeDoc;
}
...
{
	// FIXME, these should probably be hooked in another spot?
	HOOK_MESSAGE(Technology);
...
// Output : int
//-----------------------------------------------------------------------------
int CTechnologyTreeDoc::MsgFunc_Technology(const char *pszName, int iSize, void *pbuf )
{
	...
	resourcelevel = (float)READ_SHORT();
