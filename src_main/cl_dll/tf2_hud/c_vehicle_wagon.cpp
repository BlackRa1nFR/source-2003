...
void CVehicleWagonControlPanel::OnTick()
{
	...
	{
		Q_snprintf( buf, 256, "Driven by %s", pWagon->GetDriverPlayer()->GetPlayerName() );
	...
	if ( pWagon->GetOwner() == C_BaseTFPlayer::GetLocalPlayer() )
	{
		if (nPassengerCount == nMaxPassengerCount)
		{
			...
			C_BaseTFPlayer *pPlayer = static_cast<C_BaseTFPlayer*>(pWagon->GetPassenger( VEHICLE_DRIVER ));
