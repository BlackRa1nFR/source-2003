...
{
public:
	static bool Initialize(char *pszMaterialsDir, HWND hwnd);
	static void ShutDown(void);
	static void	EnumerateMaterials( IMaterialEnumerator *pEnum, int nContext, int nFlags = INCLUDE_ALL_MATERIALS );
	static CMaterial *CreateMaterial( const char *pszMaterialName, bool bLoadImmediately, bool* pFound = 0 );

	virtual ~CMaterial(void);

	void Draw(CDC *pDC, RECT& rect, int iFontHeight, int iIconHeight, DWORD dwFlags = (drawCaption|drawIcons));
...
{
	extern IMaterialSystem* g_pMaterialSystemInterface;
	return g_pMaterialSystemInterface;
}
