...
public:
	...
	inline void ToggleResubdivideState( void )	{ m_bResubdivide = !m_bResubdivide; }
	inline bool	GetResubdivideState( void )		{ return m_bResubdivide; }
...
protected:
	...
	void ApplyPaintTool( UINT nFlags, CPoint point, CMapDisp *pDisp );
	void ApplySpatialPaintTool( UINT nFlags, CPoint point, CMapDisp *pDisp );
	void LiftFaceNormal( CMapView3D *pView, CPoint point );
	...
	void HandleSelection( CMapView3D *pView, CPoint point );
	EditDispHandle_t GetHitPos( CMapView3D *pView, CPoint point );
	inline CMapDisp *GetEditDisp( void );

	void HandleTagging( CMapView3D *pView, CPoint point );
	void HandleTaggingReset( CMapView3D *pView, CPoint point );
...
private:
	...
	void GetRay( CMapView3D *pView, CPoint point, Vector& rayStart, Vector& rayEnd );
...
protected:
	...
	bool						m_bResubdivide;			// hold the resubdivide state
