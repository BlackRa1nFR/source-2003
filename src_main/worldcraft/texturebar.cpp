...
BEGIN_MESSAGE_MAP(CTextureBar, CWorldcraftBar)
...
BOOL CTextureBar::Create(CWnd *pParentWnd, int IDD, int iBarID)
{
	...
	if (!CWorldcraftBar::Create(pParentWnd, IDD, CBRS_RIGHT, iBarID))
	{
		return(FALSE);
	}
...
//-----------------------------------------------------------------------------
// Purpose: Disables the Replace button when there's no active document.
...
void wndTex::OnPaint(void)
{
	...
	dc.RealizePalette();
	m_pTexture->Draw(&dc, r, 0, 0, drawResizeAlways);
}
