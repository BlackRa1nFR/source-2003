...
{
	if(pt[0] < this->bmins[0])
		this->bmins[0] = pt[0];
	if(pt[1] < this->bmins[1])
		this->bmins[1] = pt[1];
	if(pt[2] < this->bmins[2])
		this->bmins[2] = pt[2];

	if(pt[0] > this->bmaxs[0])
		this->bmaxs[0] = pt[0];
	if(pt[1] > this->bmaxs[1])
		this->bmaxs[1] = pt[1];
	if(pt[2] > this->bmaxs[2])
		this->bmaxs[2] = pt[2];
}
...
//			bmaxs - 
//-----------------------------------------------------------------------------
void BoundBox::UpdateBounds(const Vector& bmins, const Vector& bmaxs)
{
	if(bmins[0] < this->bmins[0])
		this->bmins[0] = bmins[0];
	if(bmins[1] < this->bmins[1])
		this->bmins[1] = bmins[1];
	if(bmins[2] < this->bmins[2])
		this->bmins[2] = bmins[2];

	if(bmaxs[0] > this->bmaxs[0])
		this->bmaxs[0] = bmaxs[0];
	if(bmaxs[1] > this->bmaxs[1])
		this->bmaxs[1] = bmaxs[1];
	if(bmaxs[2] > this->bmaxs[2])
		this->bmaxs[2] = bmaxs[2];
}
...
{
	ptdest[0] = (bmins[0] + bmaxs[0])/2.0f;
	ptdest[1] = (bmins[1] + bmaxs[1])/2.0f;
	ptdest[2] = (bmins[2] + bmaxs[2])/2.0f;
}
