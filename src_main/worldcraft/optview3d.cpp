...
BOOL COPTView3D::OnInitDialog(void)
{
	...
	#ifdef SDK_BUILD
	GetDlgItem(IDC_ANIMATE_MODELS)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_MODEL_DISTANCE_STATIC)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_MODEL_DISTANCE)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_MODEL_DISTANCE_TEXT)->ShowWindow(SW_HIDE);
	#endif // SDK_BUILD
