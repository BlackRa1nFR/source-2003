...
BEGIN_DATADESC( CFuncTankTrain )
	...
	// Outputs
	DEFINE_OUTPUT(CFuncTankTrain, m_OnDeath, "OnDeath"),
...
BEGIN_DATADESC( CTankTargetChange )
	...
	// DEFINE_FIELD( CTankTargetChange, m_newTarget, variant_t ),
	DEFINE_KEYFIELD( CTankTargetChange, m_newTargetName, FIELD_STRING, "newtarget" ),
...
BEGIN_DATADESC( CTankTrainAI )
	...
	DEFINE_FIELD( CTankTrainAI, m_hTrain, FIELD_EHANDLE),
	DEFINE_FIELD( CTankTrainAI, m_hTargetEntity, FIELD_EHANDLE),
	DEFINE_FIELD( CTankTrainAI, m_soundPlaying, FIELD_INTEGER),
	DEFINE_SOUNDPATCH( CTankTrainAI, m_soundTreads ),
	DEFINE_SOUNDPATCH( CTankTrainAI, m_soundEngine ),

	DEFINE_KEYFIELD( CTankTrainAI, m_startSoundName, FIELD_STRING, "startsound" ),
	DEFINE_KEYFIELD( CTankTrainAI, m_engineSoundName, FIELD_STRING, "enginesound" ),
	DEFINE_KEYFIELD( CTankTrainAI, m_movementSoundName, FIELD_STRING, "movementsound" ),
	DEFINE_FIELD( CTankTrainAI, m_targetEntityName, FIELD_STRING),

	// Inputs
	DEFINE_INPUTFUNC( CTankTrainAI, FIELD_STRING, "TargetEntity", InputTargetEntity ),
...
{
	enginesound->PrecacheSound( STRING( m_startSoundName ) );
	enginesound->PrecacheSound( STRING( m_engineSoundName ) );
	enginesound->PrecacheSound( STRING( m_movementSoundName ) );
}
...
void CTankTrainAI::SoundEngineStart( void )
{
	...
	if ( m_startSoundName != NULL_STRING )
	{
		...
		EmitSound( filter, pTrain->entindex(), CHAN_ITEM, STRING(m_startSoundName), 1.0, ATTN_NORM, 0, 100 );
	}
