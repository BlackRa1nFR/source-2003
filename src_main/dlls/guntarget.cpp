...
BEGIN_DATADESC( CGunTarget )
	...
	DEFINE_FIELD( CGunTarget, m_hTargetEnt, FIELD_EHANDLE ),

	// Function Pointers
	DEFINE_FUNCTION( CGunTarget, Next ),
	DEFINE_FUNCTION( CGunTarget, Start ),
	DEFINE_FUNCTION( CGunTarget, Wait ),

	// Inputs
	DEFINE_INPUTFUNC( CGunTarget, FIELD_VOID, "Start", InputStart ),
	DEFINE_INPUTFUNC( CGunTarget, FIELD_VOID, "Stop", InputStop ),
	DEFINE_INPUTFUNC( CGunTarget, FIELD_VOID, "Toggle", InputToggle ),

	// Outputs
	DEFINE_OUTPUT(CGunTarget, m_OnDeath, "OnDeath"),
...
void CGunTarget::Activate( void )
{
	...
	{
		m_hTargetEnt = pTarg;
		Vector nextPos = pTarg->GetAbsOrigin() - (WorldAlignMins() + WorldAlignMaxs()) * 0.5;
...
void CGunTarget::Next( void )
{
	...
	LinearMove( pTarget->GetLocalOrigin() - (WorldAlignMins() + WorldAlignMaxs()) * 0.5, m_flSpeed );
}
