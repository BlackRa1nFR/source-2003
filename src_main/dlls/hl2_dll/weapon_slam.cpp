...
BEGIN_DATADESC( CWeapon_SLAM )
	...
	DEFINE_FIELD( CWeapon_SLAM, m_tSlamState, FIELD_INTEGER ),
	DEFINE_FIELD( CWeapon_SLAM, m_bDetonatorArmed, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_bNeedDetonatorDraw, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_bNeedDetonatorHolster, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_bNeedReload, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_bClearReload, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_bThrowSatchel, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_bAttachSatchel, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_bAttachTripmine, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeapon_SLAM, m_flWallSwitchTime, FIELD_TIME ),

	// Function Pointers
	DEFINE_FUNCTION( CWeapon_SLAM, SLAMThink ),
//	DEFINE_FUNCTION( CWeapon_SLAM, SlamTouch ),
...
void CWeapon_SLAM::SatchelDetonate()
{
	...
	while ((pEntity = gEntList.FindEntityByClassname( pEntity, "npc_satchel" )) != NULL)
	{
		...
		if (pSatchel->m_bIsLive && pSatchel->GetOwner() && GetOwner() && pSatchel->GetOwner()->pev == GetOwner()->pev)
		{
...
bool CWeapon_SLAM::AnyUndetonatedCharges(void)
{
	...
	{
		CSatchelCharge* pSatchel = dynamic_cast<CSatchelCharge *>(pEntity);
		if (pSatchel->m_bIsLive && pSatchel->GetOwner() && pSatchel->GetOwner()->pev == GetOwner()->pev)
		{
			return true;
		}
	}
