...
//	DEFINE_FIELD( CGib, m_lifeTime, FIELD_TIME ),
//	DEFINE_FIELD( CGib, m_pSprite, CSprite ),

	// Function pointers
	DEFINE_FUNCTION( CGib, BounceGibTouch ),
	DEFINE_FUNCTION( CGib, StickyGibTouch ),
	DEFINE_FUNCTION( CGib, WaitTillLand ),
...
void CGib::InitGib( CBaseEntity *pVictim, float fMinVelocity, float fMaxVelocity )
{
	...
	if ( pVictim )
	{
		...
		Vector vecOrigin;
		pVictim->RandomPointInBounds( vec3_origin, Vector( 1, 1, 1 ), &vecOrigin );
		vecOrigin.z += 1.0f;
...
CBaseEntity *CreateRagGib( const char *szModel, const Vector &vecOrigin, const QAngle &vecAngles, const Vector &vecForce )
{
	CRagGib *pGib;
	...
	pGib->Spawn( szModel, vecOrigin, vecForce );
	return pGib;
}

void CRagGib :: Spawn( const char *szModel, const Vector &vecOrigin, const Vector &vecForce )
{
	SetSolid( SOLID_BBOX );
	AddSolidFlags( FSOLID_NOT_SOLID );
	SetModel( szModel );
