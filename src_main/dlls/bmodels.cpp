...
BEGIN_DATADESC( CFuncWall )
	...
	DEFINE_FIELD( CFuncWall, m_nState,	FIELD_INTEGER ),
...
BEGIN_DATADESC( CFuncWallToggle )
	...
	DEFINE_INPUTFUNC( CFuncWallToggle, FIELD_VOID, "Toggle", InputToggle ),
...
void CFuncVehicleClip::Spawn()
{
	...
	m_fEffects |= EF_NODRAW;		// make entity invisible
...
BEGIN_DATADESC( CFuncConveyor )
	...
	DEFINE_INPUTFUNC( CFuncConveyor, FIELD_VOID, "ToggleDirection", InputToggleDirection ),
	DEFINE_INPUTFUNC( CFuncConveyor, FIELD_VOID, "SetSpeed", InputSetSpeed ),

	DEFINE_KEYFIELD( CFuncConveyor, m_vecMoveDir, FIELD_VECTOR, "movedir" ),
	DEFINE_FIELD( CFuncConveyor, m_flConveyorSpeed, FIELD_FLOAT ),
...
BEGIN_DATADESC( CFuncRotating )
	...
	DEFINE_FIELD( CFuncRotating, m_vecMoveAng, FIELD_VECTOR ),
	...
	DEFINE_FIELD( CFuncRotating, m_flTargetSpeed, FIELD_FLOAT ),
	DEFINE_KEYFIELD( CFuncRotating, m_flMaxSpeed, FIELD_FLOAT, "maxspeed" ),
	DEFINE_KEYFIELD( CFuncRotating, m_flBlockDamage, FIELD_FLOAT, "dmg" ),
	DEFINE_KEYFIELD( CFuncRotating, m_NoiseRunning, FIELD_SOUNDNAME, "message" ),
	DEFINE_FIELD( CFuncRotating, m_bReversed, FIELD_BOOLEAN ),

	// Function Pointers
	DEFINE_FUNCTION( CFuncRotating, SpinUpMove ),
	DEFINE_FUNCTION( CFuncRotating, SpinDownMove ),
	DEFINE_FUNCTION( CFuncRotating, HurtTouch ),
	DEFINE_FUNCTION( CFuncRotating, RotatingUse ),
	DEFINE_FUNCTION( CFuncRotating, RotateMove ),
	DEFINE_FUNCTION( CFuncRotating, ReverseMove ),

	// Inputs
	DEFINE_INPUTFUNC( CFuncRotating, FIELD_FLOAT, "SetSpeed", InputSetSpeed ),
	DEFINE_INPUTFUNC( CFuncRotating, FIELD_VOID, "Start", InputStart ),
	DEFINE_INPUTFUNC( CFuncRotating, FIELD_VOID, "Stop", InputStop ),
	DEFINE_INPUTFUNC( CFuncRotating, FIELD_VOID, "Toggle", InputToggle ),
	DEFINE_INPUTFUNC( CFuncRotating, FIELD_VOID, "Reverse", InputReverse ),
	DEFINE_INPUTFUNC( CFuncRotating, FIELD_VOID, "StartForward", InputStartForward ),
	DEFINE_INPUTFUNC( CFuncRotating, FIELD_VOID, "StartBackward", InputStartBackward ),
...
void CFuncRotating::Spawn( )
{
	...
	{		
		...
		SetNextThink( gpGlobals->curtime + 1.5 );	// leave a magic delay for client to start up
	}
...
void CFuncRotating::Precache( void )
{
	...
	if ( !m_NoiseRunning || strlen( szSoundFile ) == 0 )
	{
		...
		m_NoiseRunning = AllocPooledString("common/null.wav");
	}
	enginesound->PrecacheSound( STRING( m_NoiseRunning ) );
...
void CFuncRotating::RampPitchVol( void )
{
	...
	EmitSound( filter, entindex(), CHAN_STATIC, STRING(m_NoiseRunning), fvol, m_flAttenuation, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
}
...
void CFuncRotating::UpdateSpeed( float flNewSpeed )
{
	...
	if ( ( flOldSpeed == 0 ) && ( m_flSpeed != 0 ) )
	{
		...
		CPASAttenuationFilter filter( GetAbsOrigin(), m_flAttenuation );
		filter.MakeReliable();
		EmitSound( filter, entindex(), CHAN_STATIC, STRING(m_NoiseRunning), 0.01, m_flAttenuation, 0, FANPITCHMIN );
		RampPitchVol();
	}
...
void CFuncRotating::SpinUpMove( void )
{
	...
	float flNewSpeed = fabs( m_flSpeed ) + m_flMaxSpeed * m_flFanFriction;
...
bool CFuncRotating::SpinDown( float flTargetSpeed )
{
	...
	float flNewSpeed = fabs( m_flSpeed ) - 0.5 * m_flMaxSpeed * m_flFanFriction;
	if ( flNewSpeed < 0 )
	{
		flNewSpeed = 0;
	}
...
BEGIN_DATADESC( CFuncVPhysicsClip )
	...
	// Keyfields
	DEFINE_KEYFIELD( CFuncVPhysicsClip, m_iFilterName,	FIELD_STRING,	"filtername" ),
	DEFINE_FIELD( CFuncVPhysicsClip, m_hFilter,	FIELD_EHANDLE ),
...
bool CFuncVPhysicsClip::EntityPassesFilter( CBaseEntity *pOther )
{
	...
	return pFilter ? pFilter->PassesFilter(pOther) : false;
}
