...
bool CAI_PlaneSolver::MoveLimit( Navigation_t navType, const Vector &target, bool fCheckStep, AIMoveTrace_t *pMoveTrace )
{
	PLANESOLVER_PROFILE_SCOPE( CAI_PlaneSolver_MoveLimit );
	return m_pNpc->GetMoveProbe()->MoveLimit( navType, GetLocalOrigin(), target, MASK_NPCSOLID, GetNpc()->GetNavTargetEntity(), fCheckStep, pMoveTrace );
}
...
float CAI_PlaneSolver::CalculateRegulationWeight( const AIMoveTrace_t &moveTrace, float pctBlocked )
{
	...
	else
	{
		...
		if ( moveTrace.pObstruction->MyNPCPointer() != NULL )
		{
			...
			Vector2D velBlocker = ((CAI_BaseNPC *)moveTrace.pObstruction)->GetMotor()->GetCurVel().AsVector2D();
																  ...
																  float probeDist, float spanPerProbe, int probeOffset)
{
	...
	Vector probeTarget = GetLocalOrigin() + UTIL_YawToVector( yawTest ) * probeDist;
	
	if ( !MoveLimit( goal.navType, probeTarget, false, &moveTrace ) )
	{
		GenerateSuggestionFromTrace( goal, moveTrace, probeDist, arcCenter, spanPerProbe );
		return SR_OK;
	}
																   ...
																   float distClear, float probeDist, float degreesToProbe, int nProbes )
{
	...
	if ( PLANE_SOLVER_THINK_FREQUENCY == 0.0 || m_RefreshSamplesTimer.Expired() )
	{
		...
		{
			seekResult = SR_OK;
			GenerateSuggestionFromTrace( goal, directTrace, probeDist, yawGoalDir, spanPerProbe );
		}
...
void CAI_PlaneSolver::VisualizeRegulations()
{
	...
	if ((GetNpc()->m_debugOverlays & OVERLAY_NPC_STEERING_REGULATIONS) != 0)
	{
		// Compute centroid...
		Vector centroid;
		VectorAdd( GetNpc()->GetAbsMins(), GetNpc()->GetAbsMaxs(), centroid );
		centroid *= 0.5f;

		m_Solver.VisualizeRegulations( centroid );
	}
}
...
void CAI_PlaneSolver::VisualizeSolution( const Vector &vecGoal, const Vector& vecActual )
{
	if ((GetNpc()->m_debugOverlays & OVERLAY_NPC_STEERING_REGULATIONS) != 0)
	{
		...
		NDebugOverlay::Line(centroid,	actualPt, 0, 255, 0, true, 0.1f );
	}
}
...
bool CAI_PlaneSolver::Solve( const AILocalMoveGoal_t &goal, float distClear, Vector *pSolution )
{
	...
	if ( GenerateObstacleSuggestions( goal, goal.directTrace, distClear, probeDist, degreesPositiveArc, NUM_PROBES ) != SR_FAIL )
	{
		if ( RunMoveSolver( goal, goal.directTrace, degreesPositiveArc, !fVeryClose, pSolution ) )
		{
			...
			MoveLimit( goal.navType, GetLocalOrigin() + *pSolution * requiredMovement, true, &moveTrace );

			solved = !IsMoveBlocked( moveTrace );
		}
	}
