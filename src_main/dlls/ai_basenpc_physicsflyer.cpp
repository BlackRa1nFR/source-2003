...
BEGIN_DATADESC( CAI_BasePhysicsFlyingBot )
	...
	DEFINE_FIELD( CAI_BasePhysicsFlyingBot,	m_vCurrentVelocity,			FIELD_VECTOR),
	DEFINE_FIELD( CAI_BasePhysicsFlyingBot,	m_vCurrentBanking,			FIELD_VECTOR),
	DEFINE_FIELD( CAI_BasePhysicsFlyingBot,	m_vNoiseMod,				FIELD_VECTOR),
	DEFINE_FIELD( CAI_BasePhysicsFlyingBot,	m_fHeadYaw,					FIELD_FLOAT),
	DEFINE_FIELD( CAI_BasePhysicsFlyingBot,	m_vLastPatrolDir,			FIELD_VECTOR),

	// Created during CreateVPhysics
	// DEFINE_PHYSPTR( CAI_BasePhysicsFlyingBot, m_pMotionController ),
...
{
	float desYaw = UTIL_AngleDiff(VecToYaw(MoveTarget - GetLocalOrigin()), 0*GetLocalAngles().y);
...
IMotionEvent::simresult_e CAI_BasePhysicsFlyingBot::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	...
	if (!pPhysicsObject) return SIM_NOTHING;
	...
	*/
	...
	angular.x = UTIL_AngleDiff( 0 * m_vCurrentBanking.z, actualAngles.z ) * (0.1 / deltaTime) * 10.0;
	angular.y = UTIL_AngleDiff( 0 * m_vCurrentBanking.x, actualAngles.x ) * (0.1 / deltaTime) * 10.0;

	// turn toward target, add 0.2 seconds of anticipation, 90% strength
	angular.z = UTIL_AngleDiff( m_fHeadYaw, actualAngles.y + actualAngularVelocity.z * 0.2 ) * (0.1 / deltaTime) * 0.9;

	// angular = m_vCurrentAngularVelocity - actualAngularVelocity;

	// Msg("Sim %d : %.1f %.1f %.1f (%.1f)\n", count++, actualAngles.x, actualAngles.y, actualAngles.z, m_fHeadYaw );
