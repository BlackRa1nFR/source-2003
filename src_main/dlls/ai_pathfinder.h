...
public:
	...
	AI_Waypoint_t *BuildNodeRoute( const Vector &vStart, const Vector &vEnd, float goalTolerance );
	AI_Waypoint_t *BuildLocalRoute( const Vector &vStart, const Vector &vEnd, CBaseEntity const *pTarget, int endFlags, int nodeID, int buildFlags, float goalTolerance);
	AI_Waypoint_t *BuildRadialRoute( const Vector &vStartPos, const Vector &vCenterPos, float flRadius, float flArc, float flStepDist, bool bClockwise, float goalTolerance, bool bAirRoute );	
	
	AI_Waypoint_t *BuildTriangulationRoute( const Vector &vStart, 
