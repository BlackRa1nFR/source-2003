...
public:
	...
	void SetVehicleSounds( const char *pStartSound, const char *pRunningSound, const char *pStopSound, const char *pTurboSound );

	void Teleport( matrix3x4_t& relativeTransform );
	bool VPhysicsUpdate( IPhysicsObject *pPhysics );
	bool Think();
	void PlaceWheelDust( int wheelIndex );

	void DrawDebugGeometryOverlays();
	int DrawDebugTextOverlays( int nOffset );

	// Updates the controls based on user input
	void UpdateDriverControls( int nButtons, float flFrameTime );
