...
const char * g_ppszTaskFailureText[] =
{
	...
	"Weapon doesn't exist",                          // FAIL_WEAPON_NO_FIND
...
void CAI_BaseNPC::InitDefaultTaskSR(void)
{
	...
	ADD_DEF_TASK( TASK_WALK_TO_TARGET );
	ADD_DEF_TASK( TASK_RUN_TO_TARGET );
	...
	ADD_DEF_TASK( TASK_CLEAR_COMMAND_TARGET );
	...
	ADD_DEF_TASK( TASK_RANGE_ATTACK1_NOTURN );
	ADD_DEF_TASK( TASK_RANGE_ATTACK2_NOTURN );
	ADD_DEF_TASK( TASK_MELEE_ATTACK1_NOTURN );
	ADD_DEF_TASK( TASK_MELEE_ATTACK2_NOTURN );
	ADD_DEF_TASK( TASK_RELOAD_NOTURN );
	...
	ADD_DEF_TASK( TASK_GET_DROPSHIP_DEPLOY_PATH  );
