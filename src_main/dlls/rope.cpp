...
void CRopeKeyframe::Activate()
{
	...
	if ( m_iRopeMaterialModel == -1 )
		m_iRopeMaterialModel = engine->PrecacheModel( "cable/cable.vmt" );
...
bool CRopeKeyframe::KeyValue( const char *szKeyName, const char *szValue )
{
	...
	else if ( stricmp( szKeyName, "RopeShader" ) == 0 )
	{
		...
		{
			m_iRopeMaterialModel = engine->PrecacheModel( "cable/rope.vmt" );
		}
		else
		{
...
void CRopeKeyframe::SetMaterial( const char *pName )
{
	...
	m_iRopeMaterialModel = engine->PrecacheModel( STRING( stringHandle ) );
}
