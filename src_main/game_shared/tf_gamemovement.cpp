...
void CTFGameMovement::CategorizePosition( void )
{
	...
	trace_t trace;
	Vector vStart( mv->m_vecOrigin.x, mv->m_vecOrigin.y, mv->m_vecOrigin.z + 2 );
	Vector vEnd( mv->m_vecOrigin.x, mv->m_vecOrigin.y, mv->m_vecOrigin.z - 66 );
	...
	if ( mv->m_vecVelocity.z <= 180.0f )
	{
		...
		if ( ( ( trace.plane.normal.z >= IMPACT_NORMAL_FLOOR ) || trace.IsDispSurfaceWalkable() ) && ( trace.fraction < 0.06f ) )
		{
			...
			if ( ( int )player->GetWaterLevel() < WL_Waist && !trace.startsolid && !trace.allsolid )
			{
				...
				mv->m_vecOrigin = vStart + trace.fraction * ( vEnd - vStart );
...
float CTFGameMovement::ComputeConstraintSpeedFactor( void )
{
	...
	float flDistSq = mv->m_vecOrigin.DistToSqr( pTFMove->m_vecConstraintCenter );
	...
	Vector vecDelta;
	VectorSubtract( mv->m_vecOrigin, pTFMove->m_vecConstraintCenter, vecDelta );
...
void CTFGameMovement::SetupSpeed( void )
{
	...
	if (m_pSurfaceData)
	{
		flSpeedFactor = m_pSurfaceData->maxSpeedFactor;
	}
...
//	PlantFootprint( psurface );
	...
	if ( !CBaseEntity::GetParametersForSound( szSound, params ) )
		return;
...
void CTFGameMovement::HandlePlayerMove( void )
{
	...
	switch ( player->GetMoveType() )
	{
		...
		case MOVETYPE_LADDER:
			{
				FullLadderMove( false ); // the below never equated to anything but false!!
		...
		case MOVETYPE_WALK:
			{
				...
				CTFMoveData *pTFMove = TFMove();
				Vector vecPlayerOrigin( mv->m_vecOrigin.x, mv->m_vecOrigin.y, mv->m_vecOrigin.z );
				FullWalkMove( false ); // the below never equated to anything but false!!
				Vector vecPlayerDelta;
				VectorSubtract( mv->m_vecOrigin, vecPlayerOrigin, vecPlayerDelta );
...
void CTFGameMovement::AirMove( void )
{
	...
	VectorCopy( mv->m_vecOrigin, m_aMovementStack[0].m_vecPosition );
												   ...
												   float &flWishSpeed )
{
	...
	//
	vWishPos.x = mv->m_vecOrigin.x + ( mv->m_vecVelocity.x * gpGlobals->frametime );
	vWishPos.y = mv->m_vecOrigin.y + ( mv->m_vecVelocity.y * gpGlobals->frametime );
	vWishPos.z = mv->m_vecOrigin.z;
...
void CTFGameMovement::CollisionResponseNone( const trace_t &trace )
{
	...
	{
		VectorCopy( mv->m_vecOrigin, m_aMovementStack[m_nMovementStackSize].m_vecPosition );
...
int CTFGameMovement::TryPlayerMove2( void )
{
	...
	for ( int iBump = 0; iBump < BUMP_MAX_COUNT; iBump++ )
	{
		...
		//
		Vector vecWishPos;
		VectorMA( mv->m_vecOrigin, flTimeLeft, mv->m_vecVelocity, vecWishPos );

		//
		// Attempt the move.
		//
		// NOTE:  This check can hit players and objects
		TracePlayerBBoxWithStep( mv->m_vecOrigin, vecWishPos, MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER, trace );
...
void CTFGameMovement::TryStanding( void )
{
	...
	// Step down.
	Vector vecStandPos( mv->m_vecOrigin.x, mv->m_vecOrigin.y, mv->m_vecOrigin.z - player->m_Local.m_flStepSize );

	trace_t trace;
	TracePlayerBBoxWithStep( mv->m_vecOrigin, vecStandPos, MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT, trace );
	if ( trace.fraction == 1.0f )
		return;

	// Attempt to stand up.
	vecStandPos.z = mv->m_vecOrigin.z + ( ( 1.0f - trace.fraction ) * player->m_Local.m_flStepSize );
	TracePlayerBBoxWithStep( mv->m_vecOrigin, vecStandPos, MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT, trace );
	...
	{
		VectorCopy( m_aMovementStack[m_nMovementStackSize-1].m_vecPosition, mv->m_vecOrigin );
...
void CTFGameMovement::ResolveStanding( void )
{
	...
	//
	Vector vecStandPos( mv->m_vecOrigin.x, mv->m_vecOrigin.y, mv->m_vecOrigin.z - ( player->m_Local.m_flStepSize * 2.0f ) );
...
//	}
	...
	if ( trace.fraction > 0.5f )
	{
		trace.fraction -= 0.5f;
		Vector vecNewOrigin;
		vecNewOrigin = mv->m_vecOrigin + trace.fraction * ( vecStandPos - mv->m_vecOrigin );
		mv->m_vecOrigin = vecNewOrigin;
		return;
	}

	// Less than 0.5 mean we need to attempt to push up the difference.
	vecStandPos.z = ( mv->m_vecOrigin.z + ( ( 0.5f - trace.fraction ) * ( player->m_Local.m_flStepSize * 2.0f ) ) );
	TracePlayerBBoxWithStep( mv->m_vecOrigin, vecStandPos, MASK_PLAYERSOLID, COLLISION_GROUP_PLAYER_MOVEMENT, trace );
...
void CTFGameMovement::WalkMove2( void )
{
	...
	// For physics player shadow.
	float flStepHeight = mv->m_vecOrigin.z - m_aMovementStack[0].m_vecPosition.z;
