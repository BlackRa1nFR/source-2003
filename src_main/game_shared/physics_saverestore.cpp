...
BEGIN_SIMPLE_DATADESC( PhysBlockHeader_t )
	DEFINE_FIELD( PhysBlockHeader_t,	nSaved,	FIELD_INTEGER ),
	// NOTE: We want to save the actual address here for remapping, so use an integer
	DEFINE_FIELD( PhysBlockHeader_t,	pWorldObject, FIELD_INTEGER ),	
END_DATADESC()
...
BEGIN_SIMPLE_DATADESC( PhysObjectHeader_t )
  	DEFINE_FIELD( PhysObjectHeader_t,	type,		FIELD_INTEGER ),
  	DEFINE_FIELD( PhysObjectHeader_t,	hEntity,	FIELD_EHANDLE ),
  	DEFINE_FIELD( PhysObjectHeader_t,	fieldName,	FIELD_STRING ),
  	DEFINE_FIELD( PhysObjectHeader_t,	nObjects,	FIELD_INTEGER ),
  	DEFINE_FIELD( PhysObjectHeader_t,	modelName,	FIELD_STRING ),

	// Silence, Classcheck!
	// DEFINE_FIELD( PhysObjectHeader_t, bbox, BBox_t ),
	// DEFINE_FIELD( PhysObjectHeader_t, sphere, Sphere_t ),

  	DEFINE_FIELD( PhysObjectHeader_t,	bbox.mins,	FIELD_VECTOR ),
  	DEFINE_FIELD( PhysObjectHeader_t,	bbox.maxs,	FIELD_VECTOR ),
  	DEFINE_FIELD( PhysObjectHeader_t,	sphere.radius, FIELD_FLOAT ),
END_DATADESC()
...
public:
	...
	void RestorePhysicsObject( IRestore *pRestore, const PhysObjectHeader_t &header, void **ppObject, const CPhysCollide *pCollide = NULL )
	{
		if ( physenv )
		{
			physrestoreparams_t params = { pRestore, ppObject, header.type, header.hEntity.Get(), STRING(header.modelName), pCollide, physenv };
...
public:
	...
	virtual void Restore( const SaveRestoreFieldInfo_t &fieldInfo, IRestore *pRestore )
	{
		...
		g_PhysSaveRestoreBlockHandler.QueueRestore( (CBaseEntity *)fieldInfo.pOwner, fieldInfo.pTypeDesc, (void **)fieldInfo.pField, m_type );
	}
