						  ...
						  int iInitialWindYaw, float flInitialWindSpeed )
{
	...
	if (!g_pEffects->IsServer())
	{
		...
		enginesound->PrecacheSound("wind/wind_loop1.wav");
		m_pWindSound = controller.SoundCreate( -1, CHAN_STATIC, 
			"wind/wind_loop1.wav", ATTN_NONE );
...
//			UpdateWindSound( flTotalWindSpeed );
		...
		else
		{
			...
			// !!!HACKHACK - gust duration tied to the length of a particular wav file (spookwind1.wav). 
