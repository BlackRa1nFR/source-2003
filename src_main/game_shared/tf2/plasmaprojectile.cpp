...
BEGIN_DATADESC( CBasePlasmaProjectile )
	...
	DEFINE_FIELD( CBasePlasmaProjectile, m_flDamage, FIELD_FLOAT ),

	// Function Pointers
	DEFINE_ENTITYFUNC( CBasePlasmaProjectile, MissileTouch ),
...
BEGIN_PREDICTION_DATA_NO_BASE( CPlasmaProjectileShared )
	...
	DEFINE_PRED_FIELD_TOL( CPlasmaProjectileShared, m_vecSpawnPosition, FIELD_VECTOR, FTYPEDESC_INSENDTABLE, 0.125f ),
	DEFINE_PRED_FIELD_TOL( CPlasmaProjectileShared, m_vTracerDir, FIELD_VECTOR, FTYPEDESC_INSENDTABLE, 0.01f ),
	DEFINE_PRED_FIELD( CPlasmaProjectileShared, m_flSpawnTime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( CPlasmaProjectileShared, m_flDeathTime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( CPlasmaProjectileShared, m_flSpawnSpeed, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),

END_PREDICTION_DATA()

BEGIN_PREDICTION_DATA_NO_BASE( PositionHistory_t )

	DEFINE_FIELD( PositionHistory_t, m_Position, FIELD_VECTOR ),
	DEFINE_FIELD( PositionHistory_t, m_Time, FIELD_FLOAT ),
...
BEGIN_PREDICTION_DATA( CBasePlasmaProjectile )
	...
	DEFINE_PRED_TYPEDESCRIPTION( CBasePlasmaProjectile, m_Shared, CPlasmaProjectileShared ),

	DEFINE_PRED_FIELD( CBasePlasmaProjectile, m_vecAbsOrigin, FIELD_VECTOR, FTYPEDESC_PRIVATE | FTYPEDESC_OVERRIDE ),
	DEFINE_PRED_FIELD( CBasePlasmaProjectile, m_vecVelocity, FIELD_VECTOR, FTYPEDESC_PRIVATE | FTYPEDESC_OVERRIDE ),

	DEFINE_FIELD( CBasePlasmaProjectile, m_flMaxRange, FIELD_FLOAT ),

	// Predicted, but not in networking stream
	DEFINE_PRED_TYPEDESCRIPTION( CBasePlasmaProjectile, m_pPreviousPositions[0], PositionHistory_t ), 
	DEFINE_PRED_TYPEDESCRIPTION( CBasePlasmaProjectile, m_pPreviousPositions[1], PositionHistory_t ), 
	DEFINE_PRED_TYPEDESCRIPTION( CBasePlasmaProjectile, m_pPreviousPositions[2], PositionHistory_t ), 
	DEFINE_PRED_TYPEDESCRIPTION( CBasePlasmaProjectile, m_pPreviousPositions[3], PositionHistory_t ), 
	DEFINE_PRED_TYPEDESCRIPTION( CBasePlasmaProjectile, m_pPreviousPositions[4], PositionHistory_t ), 
...
bool CBasePlasmaProjectile::SimulateAndRender(Particle *pInParticle, ParticleDraw *pDraw, float &sortKey)
{
	...
	if ( pInParticle == m_pHeadParticle )
	{
		...
		TransformParticle( g_ParticleMgr.GetModelView(), vecOrigin, tPos );
		sortKey = (int) tPos.z;
	...
	for ( int i = 0; i < (MAX_HISTORY-1); i++ )
	{
		...
		TransformParticle( g_ParticleMgr.GetModelView(), vecWorldStart, vecScreenStart );
		TransformParticle( g_ParticleMgr.GetModelView(), vecWorldEnd, vecScreenEnd );
		...
		*/
		...
		Tracer_Draw( pDraw, vecScreenStart, vecScreenDelta, pParticle->m_flWidth, pParticle->m_flColor, flStartV, flEndV );
	}
...
void CBasePlasmaProjectile::Start(CParticleMgr *pParticleMgr, IPrototypeArgAccess *pArgs)
{
	...
	m_pTrailParticle->m_flColor[0] = 1.0f;
	m_pTrailParticle->m_flColor[1] = 1.0f;
	m_pTrailParticle->m_flColor[2] = 1.0f;
	m_pTrailParticle->m_flColor[3] = 1.0f;
...
BEGIN_PREDICTION_DATA( CPowerPlasmaProjectile )
	...
	DEFINE_PRED_FIELD_TOL( CPowerPlasmaProjectile, m_flPower, FIELD_FLOAT, FTYPEDESC_INSENDTABLE, 0.05f ),
