...
#define SHADER_INIT_PARAMS()			\
	void OnInitShaderParams( IMaterialVar **params, const char *pMaterialName )

#define SHADER_FALLBACK			\
	char const* GetFallbackShader( IMaterialVar** params ) const
...
#define SHADER_INIT						\
	...
	{									\
		return NUM_SHADER_MATERIAL_VARS + s_ShaderParams.Count();\
	}									\
	char const* GetParamName( int param ) const				\
	{														\
		if (param < NUM_SHADER_MATERIAL_VARS)				\
			return CBaseClass::GetParamName(param);			\
		else												\
			return s_ShaderParams[param - NUM_SHADER_MATERIAL_VARS]->GetName();	\
	}									\
	...
	{\
		if (param < NUM_SHADER_MATERIAL_VARS)				\
			...
			return s_ShaderParams[param - NUM_SHADER_MATERIAL_VARS]->GetHelp();		\
	}\
	...
	{\
		if (param < NUM_SHADER_MATERIAL_VARS)				\
			...
			return s_ShaderParams[param - NUM_SHADER_MATERIAL_VARS]->GetType();		\
	}\
	...
	{\
		if (param < NUM_SHADER_MATERIAL_VARS)				\
			...
			return s_ShaderParams[param - NUM_SHADER_MATERIAL_VARS]->GetDefault();		\
	}\
