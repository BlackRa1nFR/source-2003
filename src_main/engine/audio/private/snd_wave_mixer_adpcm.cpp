...
bool CAudioMixerWaveADPCM::DecodeBlock( void )
{
	...
	int available = m_pData->ReadSourceData( (void **) (&pData), m_offset, m_blockSize, copyBuf );
	if ( available < m_blockSize )
	{
		...
		while ( available && total < m_blockSize )
		{
			...
			total += available;
			available = m_pData->ReadSourceData( (void **) (&pData), m_offset + total, m_blockSize - total, copyBuf );
		}
		pData = tmpBlock;
		available = total;
	}
...
// Input  : delaySamples - 
//-----------------------------------------------------------------------------
void CAudioMixerWaveADPCM::SetStartupDelaySamples( int delaySamples )
{
